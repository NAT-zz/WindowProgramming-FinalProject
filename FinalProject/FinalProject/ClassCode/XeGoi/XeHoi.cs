...
namespace FinalProject
{
    class XeHoi
    {
        ...
        public bool ThemXe(string id, string nguoiGoi, string hinhThucGoi, DateTime ngayGoi, MemoryStream picBangSo, MemoryStream picHieuXe, string hieuXe, string bangSo)
        {
            DateTime ngayNhan = NgayToiHan(hinhThucGoi, ngayGoi);
            command = new SqlCommand("insert into XeHoi (MaXe,NguoiGoi,HinhThucGoi,NgayGoi,NgayToiHan,AnhBangSo,AnhHieuXe,HieuXe,BangSo)" +
                "values(@id,@name,@hinhthuc,@ngoi,@nnhan, @pcbs, @pchx, @hx, @bs", mydb.getConnection);
            ...
            command.Parameters.AddWithValue("@pchx", picHieuXe);
            command.Parameters.AddWithValue("@hx", hieuXe);
        ...
        public bool SuaXe(string id, string nguoiGoi, string hinhThucGoi, DateTime ngayGoi, MemoryStream picBangSo, MemoryStream picHieuXe, string hieuXe, string bangSo)
        {
            DateTime ngayNhan = NgayToiHan(hinhThucGoi, ngayGoi);

            command = new SqlCommand("update XeMay1 set NguoiGoi=@name,HinhThucGoi=@hinhthuc,NgayGoi=@ngoi,NgayToiHan=@nnhan,AnhBangSo=@pcbs,AnhHieuXe=@pchx,HieuXe=@hx,BangSo=@bs where MaXe=@id", mydb.getConnection);
